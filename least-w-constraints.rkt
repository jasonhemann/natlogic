...
(define LLeast
  (lambda (phi gamma proof)
    (fresh (a b)
      (== `(Least as many ,a ,b) phi) (un-atomo a) (un-atomo b)
      (conde
        ((fresh (c r1 r2)
          (== `((,r1 ,r2) => ,phi) proof) (un-atomo c)
...
(define LAll
  (lambda (phi gamma proof)
    (fresh (a b)
      (== `(All ,a are ,b) phi) (un-atomo a) (un-atomo b)
      (conde
        ((== a b) (== phi proof))
        ((fresh (r1 r2)
           (== `((,r1 ,r2) => ,phi) proof)
        ...
        ((fresh (c r1 r2) (un-atomo c)
           (== `((,r1 ,r2) => ,phi) proof)  
           (=/= a c)
           (=/= b c)
           (L `(All ,a are ,c) gamma r1)
           (L `(All ,c are ,b) gamma r2)))
        ((fresh (p q r1 r2) (un-atomo p) (un-atomo q)
           (== `((,r1 ,r2) => ,phi) proof)
...
(define LSome
  (lambda (phi gamma proof)
    (fresh (a b) (un-atomo a) (un-atomo b)
      (== `(Some ,a are ,b) phi)
      (conde
        [(== a b)
         (fresh (c r1 r2) (un-atomo c)
